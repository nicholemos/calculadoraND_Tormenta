...
function sortTableByDifficulty() {
	var creatureList = document.getElementById("creatureList");
	var rows = creatureList.getElementsByTagName("tr");
	var headerRow = rows[0]; // Linha de cabeçalho
	var sortedRows = Array.from(rows).slice(1); // Exclua a primeira linha de cabeçalho da tabela

	sortedRows.sort(function (a, b) {
		var difficultyA = parseFloat(a.getElementsByTagName("td")[5].innerText);
		var difficultyB = parseFloat(b.getElementsByTagName("td")[5].innerText);

		return difficultyB - difficultyA; // Ordem decrescente
	});

	// Limpe a tabela, mantendo a linha de cabeçalho
	while (creatureList.firstChild) {
		creatureList.removeChild(creatureList.firstChild);
	}

	// Adicione a linha de cabeçalho de volta à tabela
	creatureList.appendChild(headerRow);

	// Adicione as linhas classificadas de volta à tabela
